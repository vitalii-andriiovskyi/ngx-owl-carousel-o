...
export class AutoplayService implements OnDestroy {
  ...
  spyDataStreams() {
    ...
    const resized$: Observable<any> = this.carouselService
      .getResizedState()
      .pipe(
        tap(() => {
          if (
    ...
    const autoplayMerge$: Observable<string> = merge(
      initializedCarousel$,
      changedSettings$,
      resized$
    );
      ...
      return this.winRef.setTimeout(() => {
        this.ngZone.run(() => {
          ...
          this.carouselService.next(
            speed || (this.carouselService.settings.autoplaySpeed as number)
          );
        });
