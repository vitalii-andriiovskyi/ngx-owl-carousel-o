...
import {
  OwlCarouselOConfig,
  OwlOptionsMockedTypes,
} from '../carousel/owl-carousel-o-config';
...
export class CarouselService {
  ...
  private _pipe: any[] = [
    ...
    // },
    {
      ...
      run: (cache: any) => {
        cache.current =
          this._items &&
          this._items[this.relative(this._current as number)]?.id();
      },
    },
    // {
    ...
    },
    {
      ...
      run: () => {
        ...
        begin = this.coordinates(this.current()) as number;
        ...
        for (i = 0, n = this._coordinates.length; i < n; i++) {
          ...
          if (
            (this._op(inner, '<=', begin) && this._op(inner, '>', end)) ||
  ...
  setOptions(options: OwlOptions) {
    ...
    const checkedOptions: OwlOptions = this._validateOptions(
      options,
      configOptions
    );
  ...
  private _validateOptions(
    options: OwlOptions,
    configOptions: OwlOptions
  ): OwlOptions {
    const checkedOptions = { ...(options || {}) } as any;
    const mockedTypes = new OwlOptionsMockedTypes();

    const setRightOption = (
      type: string,
      key: keyof OwlOptions
    ): OwlOptions[keyof OwlOptions] => {
      this.logger.log(
    ...
    Object.entries(checkedOptions).forEach(([key, value]: [string, any]) => {
      const typedKey: keyof OwlOptions = key as keyof OwlOptions;
        ...
        if (this._isNumeric(value)) {
          checkedOptions[typedKey] =
            typedKey === 'items'
              ? this._validateItems(
                  +value,
                  checkedOptions.skip_validateItems as boolean
                )
              : +value;
        } else {
          checkedOptions[typedKey] = setRightOption(
            mockedTypes[typedKey] as string,
            typedKey
          );
        }
      } else if (mockedType === 'boolean' && typeof value !== 'boolean') {
        checkedOptions[key] = setRightOption(
          mockedTypes[typedKey] as string,
          typedKey
        );
      } else if (
        mockedType === 'number|boolean' &&
    ...
    this.settings = {
      ...
      items:
        overwrites[match] && overwrites[match].items
          ? this._validateItems(
              overwrites[match].items as number,
              this._options.skip_validateItems as boolean
            )
          : this._options.items,
    };
  ...
  update() {
    ...
    const n = this._pipe.length,
      filter = (item: any) => this._invalidated[item],
      cache = {};
  ...
  prepareDragging(event: any): Coords {
    let stage: Coords, transformArr: string[];
  ...
  defineNewCoordsDrag(event: any, dragData: any): boolean | Coords {
    ...
    if (this.settings.loop) {
      ...
      minimum = this.settings.rtl
        ? (this.coordinates(this.maximum()) as number)
        : (this.coordinates(this.minimum()) as number);
  ...
  closest(coordinate: number, direction: string | null): number {
    ...
    for (let i = 0; i < coordinates.length; i++) {
      if (
        ...
        this._op(coordinate, '<', coordinates[i]) &&
  ...
  is(state: string): boolean {
    return (
      (this._states.current as any)[state] &&
      (this._states.current as any)[state] > 0
    );
  }
  ...
  setCurSlideClasses(slide: SlideModel): { [key: string]: boolean } {
    ...
    if (this.settings.animateIn) {
      currentClasses[this.settings.animateIn as string] =
        slide.isCustomAnimatedIn || false;
    }
    if (this.settings.animateOut) {
      currentClasses[this.settings.animateOut as string] =
