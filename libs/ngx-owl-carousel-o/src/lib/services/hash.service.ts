...
export class HashService implements OnDestroy {
  ...
  spyDataStreams() {
      ...
      .pipe(
        tap((data) => {
          if (
            ...
            const newCurSlide = this.carouselService.current() as number;
            const newCurFragment =
              this.carouselService.slidesData[newCurSlide].hashFragment;
    ...
    const hashFragment$: Observable<string | any> = merge(
      initializedCarousel$,
      changedSettings$
    );
  ...
  listenToRoute() {
    ...
    this.route.fragment.pipe(skip(count)).subscribe((fragment) => {
      this.currentHashFragment = fragment as string;
      this.rewind(fragment as string);
    });
  }
}
