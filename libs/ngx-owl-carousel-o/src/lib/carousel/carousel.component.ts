...
import {
  ...
  DotsData,
  OwlSingeDot,
} from '../models/navigation-data.models';
...
export class CarouselComponent implements OnInit, OnDestroy, AfterContentInit {
   ...
   */
  private _options$ = toObservable<OwlOptions>(
    this.options as Signal<OwlOptions>
  );
  /**
  ...
  spyDataStreams() {
    ...
    this._changedCarousel$ = this.carouselService.getChangeState().pipe(
      switchMap((value) => {
        const changedPosition: Observable<SlidesOutputData> = of(value).pipe(
          ...
          map((slide) => {
            ...
            const id =
              slide.id.indexOf(clonedIdPrefix) >= 0
      ...
      tap((slidesData) => {
        this.gatherTranslatedData();
        this.changed.emit(
          slidesData?.slides?.length ? slidesData : this.slidesOutputData
        );
    ...
    this._draggingCarousel$ = this.carouselService.getDragState().pipe(
      ...
      switchMap(() =>
        this.carouselService
          .getDraggedState()
          .pipe(map(() => !!this.carouselService.is('animating')))
      ),
      switchMap((anim) => {
        if (anim) {
  ...
  next() {
    ...
    this.navigationService.next(
      this.carouselService.settings.navSpeed || false
    );
    return false;
  }
